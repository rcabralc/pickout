...
cdef int [:,:,:] create_fuzzy_search_data(int p_length, int v_length):
	return cvarray(
		shape=(2, p_length, v_length + 1),
		itemsize=sizeof(int),
		format='i'
	)
cdef int [:,:,:] GLOBAL_M = create_fuzzy_search_data(
	PATTERN_GLOBAL_MAX_LENGTH,
...
class Filter:
		...
		for patternType in patternTypes:
			if pattern.startswith(patternType.prefix):
				if patternType == RegexPattern:
					return patternType(
						pattern[len(patternType.prefix):],
						ignore_bad_patterns=ignore_bad_patterns
					)
	...
	def _get_matches(
			self,
			entries,
			CompositePattern pattern not None,
